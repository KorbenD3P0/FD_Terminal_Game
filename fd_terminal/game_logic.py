...
class GameLogic:
    ...
    def get_save_slot_info(self, slot_id):
        ...
        try:
            with open(save_path, "r", encoding="utf-8") as f:
                data = json.load(f).get("save_info", {}) # Get the 'save_info' sub-dictionary
            return {
    ...
    def process_player_input(self, command_str):
        ...
        verb_aliases = {
            ...
            "force": self.game_data.ACTION_BREAK, # Alias "force" to "break"
    ...
    def _command_use(self, words):
        ...
        item_type = item_master_data.get("type")
        current_room_name = self.player['location']
        hazard_specific_interaction_occurred = False
        targeted_hazard_instance = None
        targeted_hazard_id = None

        # 1. Hazard-specific interaction (by item_used_type)
        if target_object_str and self.hazard_engine:
            ...
            if targeted_hazard_instance and item_type:
                ...
                if not isinstance(hazard_interaction_rules, list):
                    hazard_interaction_rules = [hazard_interaction_rules]
        ...
        # 2. Special case: Bludworth's House Key on front door
        if target_object_str and item_in_inventory_cased == "Bludworth's House Key" and \
        ...
        # 3. Generic item-on-target logic
        elif target_object_str:
        ...
        # 5. Final hazard check for "use" action (indirect effects)
        if self.hazard_engine and not death_triggered and target_object_str:
            final_target_for_hazard_check = target_object_str
            if hazard_specific_interaction_occurred and targeted_hazard_instance:
    ...
    def _handle_qte_response(self, qte_type_from_player_state, user_input_str):
        ...
        if is_success:
            ...
            next_hazard_state_on_success = qte_context.get('next_state_for_hazard')
            if source_hazard_id and next_hazard_state_on_success and self.hazard_engine:
                ...
                if source_hazard_id in self.hazard_engine.active_hazards:
                    ...
                    if hazard_progression_messages: response["message"] += "\n" + "\n".join(hazard_progression_messages)
                else: logger.warning(f"QTE success: Source hazard {source_hazard_id} no longer active.")
            
            if qte_context.get("on_success_level_complete"): # Check if QTE success leads to level completion
            ...
            is_fatal = qte_context.get("is_fatal_on_failure", True)
            if is_fatal:
                response["death"] = True; self.is_game_over = True; self.game_won = False
                ...
                if self.player['hp'] <= 0:
                    response["death"] = True; self.player['last_death_message'] = response["message"]
            if not self.is_game_over:
                source_hazard_id = qte_context.get('qte_source_hazard_id')
                ...
                if source_hazard_id and next_hazard_state_on_failure and self.hazard_engine:
                    ...
                    if source_hazard_id in self.hazard_engine.active_hazards:
                        ...
                        if hazard_progression_messages: response["message"] += "\n" + "\n".join(hazard_progression_messages)
                    else: logger.warning(f"QTE failure: Source hazard {source_hazard_id} no longer active.")
        ...
        if self.is_game_over and not response.get("death"): response["death"] = True
        return response
